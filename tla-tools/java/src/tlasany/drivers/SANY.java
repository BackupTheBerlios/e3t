...
public class SANY 
{
  ...
  private static boolean doParsing          = RuntimeConfiguration.get().doParsing();  // true <=> parsing should be done
                                                     //          currently there is no way to turn this off

  private static boolean doSemanticAnalysis = RuntimeConfiguration.get().doSemanticAnalysis();  // true <=> semantic analysis should be done
                                                     //          default is true; turned OFF by -S switch

  private static boolean doLevelChecking    = RuntimeConfiguration.get().doLevelChecking(); // true <=> level checking should be done
                                                     //          no effect if doSemanticAnalysis is false
                                                     //          default is true; turned OFF by -L switch

  private static boolean doDebugging        = RuntimeConfiguration.get().doDebugging(); // true <=> control should be transferred to debugger;
                                                     //          default is false; turned ON by -D switch

  private static boolean doStats            = RuntimeConfiguration.get().doStats(); // true <=> statistics about builtin operator usage should
                                                     //          be reported
   ...
   * suitable for printing on System.out or RuntimeConfiguration.get().getErrStream() to the
   ...
   */
  public static final void frontEndMain(SpecObj spec, 
					String fileName, 
    ...
    try {
      ...
      if (doSemanticAnalysis) frontEndSemanticAnalysis(spec, syserr, doLevelChecking);
    }
  ...
  throws ParseException, AbortException {
    try {
      ...
      if (!spec.loadSpec(spec.getFileName(), spec.parseErrors)) {
        spec.parseErrors.addError(Location.nullLoc,
   ...
   * ExternalModuleTable.
   */
  public static void SANYmain(Vector filenames) 
  {
    ...
    for ( int i = 0; i < filenames.size(); i++) 
    {
      ...
      // Print documentation line on RuntimeConfiguration.get().getOutStream()
      RuntimeConfiguration.get().getOutStream().println("\n****** SANY 0.991 ( Created: Wed Jun 18 13:33:55  2003 )\n");
      ...
      // spec.
      SpecObj spec = new SpecObj(name);
      try 
      {
        frontEndMain(spec, name, RuntimeConfiguration.get().getOutStream());
      }
      	catch (FrontEndException fe) 
      {
      	  fe.printStackTrace(RuntimeConfiguration.get().getErrStream());
      	  return;
      }
