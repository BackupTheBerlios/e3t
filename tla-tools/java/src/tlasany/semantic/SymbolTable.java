...
// The Symbol Table builds the stack of context tables. The stack model 
// for symbol resolution is straightforward, but not necessarily very 
// efficient as we may create and push a lot of contexts as inner modules contexts, 
// LET expression contexts, bound variable contexts, and formal parameter contexts.

// The context at the bottom of the stack initially is a copy of the built-in symbols. 
// All top level definitions of the module will be added to it, to become eventually 
// the global context of this module (including symbols added by Extends and Instance)

public class SymbolTable {

   
  private Stack               contextStack;  // A SymbolTable is a stack of contexts.  New contexts are
                                             // pushed on the stack for LET, for formal param lists, 
                                             // for bound variables sets, and for inner modules
  private Context             topContext;    // The top context on the stack
  private Context             baseContext;   // The bottom context on the stack; holds builtin op symbols
                                             //   and also all top-level symbols of External module
  private ExternalModuleTable mt;            // The ExternalModuleTable for the spec to which THIS
                                             //   SymbolTable and its external module belongs
  private ModuleNode          modNode;       // The ModuleNode of the module (internal or external)
                                             //   with which THIS SymbolTable is associated
  private ErrorsContainer     errors;        // The Errors object to which error messages involving
                                             //   THIS SymbolTable should be appended.

  // This form for external modules
  public SymbolTable( ExternalModuleTable mt, ErrorsContainer errs ) {
  ...
  public SymbolTable(ExternalModuleTable mt, ErrorsContainer errs, SymbolTable st) {
    modNode = st.modNode;
    errors = errs;
    contextStack = new Stack();
  ...
  public final boolean addSymbol(UniqueString name, SymbolNode symbol) {
    ...
    RuntimeConfiguration.get().getOutStream().println("Module '" +
		       ((OpDefOrDeclNode)currentBinding).getOriginallyDefinedInModuleNode().getName() +
		       "' parameter free?  " +
		       ((OpDefOrDeclNode)currentBinding).getOriginallyDefinedInModuleNode().isParameterFree());
    */
