...
public class SpecObj {
  ...
  String primaryFileName;             // The raw file name for the root (top) module, unprocessed by adding 
                                      // ".tla" or by prepending the full file system path to it)

  ExternalModuleTable externalModuleTable = new ExternalModuleTable();
                                      // This is the one ExternalModuleTable for the entire specification; it includes
                                      // ModuleNode's for the root module, and for all modules that it depends on
                                      // directly or indirectly by EXTENDS or INSTANCE
  ...
  ParseUnit rootParseUnit = null;     // The ParseUnit object for the first (i.e. "root") file, roughly
                                      // the file that is named in the command line; null until the root file is parsed

  ModulePointer rootModule = null;    // The top level module of the rootParseUnit; null until rootParseUnit is parsed

  String rootModuleName;              // The String name of rootModule, unknown until rootParseUnit is parsed, although
                                      // it is supposed to be closely related to the file name

  public ErrorsContainer initErrors = new ErrorsContainer();
                                      // The Errors object for reporting errors that happen at initialization time

  public ErrorsContainer parseErrors = new ErrorsContainer();
                                      // The Errors object for reporting errors that occur during parsing, including
                                      // the retrieval of files (ParseUnits) for extention and instantiation or the root
                                      // and their extentions and instantiations, etc.

  ErrorsContainer globalContextErrors = new ErrorsContainer();
                                      // The Errors object for reporting errors in creating the global context from
                                      // the file that stores it.

  public ErrorsContainer semanticErrors = new ErrorsContainer();
                                      // The Errors object for reporting errors discovered during semantic analysis, 
                                      // including level checking.

  public int errorLevel = 0;

  /**
   * Constructor of the spec object
   * @param pfn
   */
  public SpecObj(String primaryFileName) 
  { 
   ...
   */
  public final ErrorsContainer getInitErrors() 
  { 
      return initErrors; 
  }
   ...
   */
  public final ErrorsContainer getParseErrors() 
  { 
      	return parseErrors; 
  }
   ...
   */
  public final ErrorsContainer getGlobalContextErrors() 
  { 
   ...
   */
  public final ErrorsContainer getSemanticErrors() 
  { 
  ...
  // neither happens.
  private final ParseUnit findOrCreateParsedUnit(String name, 
          ErrorsContainer errors, boolean firstCall)
  	throws AbortException 
  {
    ...
    if ( parseUnit == null ) 
    {
      // if module "name" is not already in parseUnitContext table
        ...
        try 
        {
            parseUnit = ParseUnitFactory.createParseUnit(name, this);
        } catch (ParseUnitCreationException e) 
        {
    ...
    return parseUnit;  // return a non-null "parseUnit" iff named module has been found,
  ...
  private void nonCircularityTest(ParseUnit parseUnit, ErrorsContainer errors ) 
  	throws AbortException 
  {
  ...
  // multiple times.
  private void nonCircularityBody(ParseUnit parseUnit, ParseUnit candidate, ErrorsContainer errors, 
    ...
    for ( int i = 0; i < referencees.size(); i++ ) {
      ...
      // See if our search has reached "parseUnit"; if so, we have a circularity
  ...
  String        nextParseUnitName = "";                // The place where name of the next unresolved extention 
                                                       // or instance module is stored by the next methods

  ModulePointer nextExtenderOrInstancerModule = null;  // The place where a reference to the module that extends
  ...
  private boolean findNextUnresolvedExtentionBody(ModulePointer currentModule, HashSet alreadyVisited) {
    ...
    // See if one of the already-resolved extendees has any unresolved extentions
    for ( int i = 0; i < extendees.size(); i++ ) {
      // by recursive invocation of this method on the extendees
      if ( findNextUnresolvedExtentionBody(currentContext.resolve((String)extendees.elementAt(i)),
                                           alreadyVisited) ) {
        extentionFound = true;
        return true;
      }
    ...
    for (int i = 0; i < innerModules.size(); i++) {
      if ( findNextUnresolvedExtentionBody((ModulePointer)innerModules.elementAt(i), 
                                           alreadyVisited) ) {
        extentionFound = true;
        return true;
      }
  ...
  private boolean instanceResolvesToInternalModule(ModulePointer currentModule, String instanceeName) {
    ...
    for (int i = 0; i < body.heirs().length; i++) {
      ...
      // if we encounter an INSTANCE decl
        ...
        // The modifier "LOCAL" may or may not appear in the syntax tree; if so, offset by 1
  ...
  private boolean findNextUnresolvedInstantiationBody(ModulePointer currentModule,
    ...
    // See if one of the already-resolved extendees has any unresolved instantiations
    for ( int i = 0; i < extendees.size(); i++ ) {
      // by recursive invocation of this method on the extendees
    ...
    Vector innerModules = currentModule.getDirectInnerModules();
    for (int i = 0; i < innerModules.size(); i++) {
      if (findNextUnresolvedInstantiationBody((ModulePointer)innerModules.elementAt(i),
					      alreadyVisited)) {
        instantiationFound = true;
        return true;
      }
    ...
    // Iff there are no unresolved Instantiation module names, we return false
    instantiationFound = false;
    return false;
  }
  ...
  private boolean directlyExtends(ModulePointer mod1, ModulePointer mod2) {
    ...
    for (int i = 0; i < extendees.size(); i++) {
      if ( mod1Context.resolve((String)extendees.elementAt(i)) == mod2 ) return true;
    }
    return false;
  }
  ...
  private Vector getModulesIndirectlyExtending(ModulePointer module) {
    // The Vector of Modules that equals, or directly or indirectly extends, "module"
    ...
    while ( additions ) {
      ...
      // for all newly added modules, see if there are any others that extend them
  ...
  private void resolveNamesBetweenModuleAndExtention(ModulePointer extenderModule,
						     ParseUnit extendeeParseUnit) {
   ...
   */
  public boolean loadSpec(String rootExternalModuleName, ErrorsContainer errors)
  throws AbortException { 
    ...
    while (findNextUnresolvedExtention(rootModule) ||
      ...
      // nextParseUnitName has not already been processed through some other path,
	...
	nextExtentionOrInstantiationParseUnit = 
          findOrCreateParsedUnit( nextParseUnitName, errors, false /* not first call */ );
      }
      else {
	// or find it in the known parseUnitContext
      ...
      if (extentionFound) {
        resolveNamesBetweenSpecAndExtention(nextExtenderOrInstancerModule, nextExtentionOrInstantiationParseUnit);
      }
      ...
      if ( instantiationFound ) {
        resolveNamesBetweenSpecAndInstantiation(nextExtenderOrInstancerModule, nextExtentionOrInstantiationParseUnit);
      }
    } // end while
    ...
    return true; //loadUnresolvedRelatives(moduleRelationshipsSpec, rootModule, errors); 
  }
