...
// Last modified on Fri Aug 10 15:07:25 PDT 2001 by yuanyu
...
public class FcnRcdValue extends Value implements Applicable {
  ...
  private final void createIndex() {
    if (this.domain != null && this.domain.length > 10) {
      int len = this.domain.length * 2;

      int[] tbl = new int[len];
      for (int i = 0; i < len; i++) { tbl[i] = -1; }
  ...
  public final int compareTo(Object obj) {
    ...
    if (this.intv != null) {
      if (fcn.intv != null) {
	int result = this.intv.compareTo(fcn.intv);
	...
	int result = this.intv.size() - fcn.domain.length;
	if (result != 0) return result;
  ...
  public final boolean equals(Object obj) {
    FcnRcdValue fcn = convert(obj);
    if (fcn == null) {
      if (obj instanceof ModelValue) return false;
	...
	for (int i = 0; i < fcn.domain.length; i++) {
	  ...
	  if (((IntValue)dElem).val != i + this.intv.low ||
  ...
  public final Value takeExcept(ValueExcept ex) {
    ...
    if (arg instanceof TupleValue) {
      Value[] elems = ((TupleValue)arg).elems;
      arg = (elems.length == 1) ? elems[0] : arg;
    }
    if (this.intv != null) {
      // domain is represented as an integer interval:
      if (arg instanceof IntValue) {
  ...
  public final Value getDomain() {
    ...
    return new SetEnumValue(this.domain, this.isNormalized());
  }

  public final Value[] getDomainVals() {
    if (domain == null) {
      domain = new Value[this.values.length];
      int j = 0;
      for(int i = intv.low; i <= intv.high; i++) {
	domain[j++] = IntValue.gen(i);
      }
    }
    return domain;
  }
  ...
  public final Value permute(MVPerm perm) {
	...
	return new FcnRcdValue(this.domain, vals, this.isNorm);
      }
    }
    else {
      if (vchanged) {
