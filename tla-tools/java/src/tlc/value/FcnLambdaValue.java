...
// Last modified on Thu Dec  6 21:46:34 PST 2001 by yuanyu
...
public class FcnLambdaValue extends Value implements Applicable {
  ...
  public final Value apply(Value arg, int control) {
    if (this.fcnRcd == null) {
      Assert.fail("Error(TLC): Should always use the other apply method.");
    }
    return this.fcnRcd.apply(arg, control);
  }
  ...
  public final Value apply(Value[] args, int control) throws EvalException {
	...
	if (args.length == 1) {
	  inExcept = arg.equals(args[0]);
	}
	else {
	  Value[] argList = ((TupleValue)arg).elems;
	  for (int j = 0; j < args.length; j++) {
	    if (!(args[j].equals(argList[j]))) {
	      inExcept = false;
	      break;
	    }
	  }
    ...
    if (res == null) {
      Context c1 = this.con;
      OpDeclNode[][] formals = this.params.formals;
      ...
      if (plen == 1 && alen > 1) {
	Value argVal = new TupleValue(args);
	if (!domains[0].member(argVal)) {
	  Assert.fail("In applying the function\n" + Value.ppr(this.toString()) +
		      ",\nthe argument is not in its domain:\n" + argVal);
	}
	if (isTuples[0]) {
	  OpDeclNode[] ids = formals[0];
	  if (ids.length != alen) {
	    Assert.fail("In applying the function\n" + Value.ppr(this.toString()) +
			",\nthe argument does not match its formal parameter.\n" + argVal);
	  }
	  for (int i = 0; i < alen; i++) {
	    c1 = c1.cons(ids[i], args[i]);
	  }
	}
	else {
	...
	Value[] elems = args;
	if (plen > 1 && alen == 1) {
	  TupleValue tv = TupleValue.convert(args[0]);
	  if (tv == null) {
  ...
  public final Value getDomain() {
    ...
    for (int i = 0; i < dlen; i++) {
      OpDeclNode[] formal = this.params.formals[i];
