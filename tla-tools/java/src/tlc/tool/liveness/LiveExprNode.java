...
// Last modified on Sun Aug  5 00:00:56 PDT 2001 by yuanyu
...
abstract class LiveExprNode {
  ...
  public final boolean equals(LiveExprNode exp) {
    ...
    return ((exp instanceof LNBool) &&
	    ((LNBool)this).b == ((LNBool)exp).b);
  }
  ...
  public final LiveExprNode simplify() {
    ...
    if (this instanceof LNAll) {
      ...
      if (body1 instanceof LNBool) return body1;
	...
	if (elem instanceof LNBool) {
	  if (((LNBool)elem).b) return new LNBool(true);
	}
	else {
	  lnd1.addDisj(elem);
	}
	...
	if (elem instanceof LNBool) {
	  if (!((LNBool)elem).b) return new LNBool(false);
	}
	else {
	  lnc1.addConj(elem);
	}
