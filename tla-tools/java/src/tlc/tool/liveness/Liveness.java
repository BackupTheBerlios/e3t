...
// Last modified on Fri Jan  4 00:31:02 PST 2002 by yuanyu
...
import tlc.tool.Action;
import tlc.tool.BuiltInOPs;
import tlc.tool.ContextEnumerator;
import tlc.tool.EvalControl;
import tlc.tool.TLCState;
import tlc.tool.Tool;
import tlc.tool.ToolGlobals;
...
public class Liveness implements ToolGlobals, ASTConstants {
  ...
  private static LiveExprNode astToLiveAppl(Tool tool, OpApplNode expr, Context con) {
    ...
    case OPCODE_sf:     // SF
      {
	// translate SF_e(A) into <>[]-EN<A>_e \/ []<><A>_e
    ...
    case OPCODE_wf:     // WF
      {
	// translate WF_e(A) into []<>(-EN<A>_e \/ <A>_e)
  ...
  public static OrderOfSolution[] processLiveness(Tool tool, String metadir) {
    ...
    if (lexpr == null) { return new OrderOfSolution[0]; }
    ...
    // tableau method. The first step is to gather everything into
    ...
    // SF will be broken up into lots of cases and the TF will remain
    // the same throughout. (Incidentally, we're checking equality on
    // TFs just syntactically.  This is sufficient, because we haven't
    // done any real rearrangement of them, apart from munging up \/
    // and /\ above them.  tfbin contains the different tf's. pembin
    // is a vect of vect-of-pems collecting each tf's pems.
    ...
    OrderOfSolution[] os = new OrderOfSolution[tfbin.size()];
    for (int i = 0; i < tfbin.size(); i++) {
      os[i] = new OrderOfSolution();
      LiveExprNode tf = tfbin.exprAt(i);
      if (tf == null) {
        os[i].tableau = null;
        os[i].promises = new LNEven[0];
      }
      else {
        LiveExprNode tf1 = tf.makeBinary();
	os[i].tableau = constructTableau(tf1, 0);
        TBPar promises = new TBPar(10);
        tf1.extractPromises(promises);
        os[i].promises = new LNEven[promises.size()]; 
	for (int j = 0; j < promises.size(); j++) {
	  os[i].promises[j] = (LNEven)promises.exprAt(j);
	}
      }

      // We lump all the pems into a single checkState and checkAct,
      // and os[i].pems will simply be integer lookups into them.
      Vect stateBin = new Vect();
      Vect actionBin = new Vect();
      Vect tfPems = (Vect)pembin.elementAt(i);
      os[i].pems = new PossibleErrorModel[tfPems.size()];
      for (int j = 0; j < tfPems.size(); j++) {
        OSExprPem pem = (OSExprPem)tfPems.elementAt(j);
        os[i].pems[j] = new PossibleErrorModel();
	os[i].pems[j].AEAction = addToBin(pem.AEAction, actionBin);
        os[i].pems[j].AEState = addToBin(pem.AEState, stateBin);
        os[i].pems[j].EAAction = addToBin(pem.EAAction, actionBin);
      }
      // Finally, store the bins with the order of solution.
      os[i].checkState = new LiveExprNode[stateBin.size()];
      for (int j = 0; j < stateBin.size(); j++) {
	os[i].checkState[j] = (LiveExprNode)stateBin.elementAt(j);	
      }
      os[i].checkAction = new LiveExprNode[actionBin.size()];    
      for (int j = 0; j < actionBin.size(); j++) {
	os[i].checkAction[j] = (LiveExprNode)actionBin.elementAt(j);
      }
    }

    RuntimeConfiguration.get().getOutStream().println("Implied-temporal checking--satisfiability problem has " +
		       os.length + " branches.");
  ...
  public static void printTBGraph(TBGraph tableau) {
    if (tableau == null) {
      RuntimeConfiguration.get().getOutStream().println("No tableau.");
    }
    else {
      RuntimeConfiguration.get().getOutStream().println(tableau.toString());
    }
  }
