...
// Last modified on Fri Jan  4 00:30:06 PST 2002 by yuanyu
...
import tlc.tool.StateVec;
import tlc.tool.TLCState;
import tlc.tool.TLCStateInfo;
import tlc.tool.TLCTrace;
import tlc.tool.Tool;
import tlc.util.LongObjTable;
import tlc.util.MemObjectStack;
import tlc.util.ObjectStack;
...
public class LiveCheck1 {
  ...
  public synchronized static void check() {
    ...
    for (int soln = 0; soln < slen; soln++) {
      ...
      // RuntimeConfiguration.get().getErrStream().println(bgraphs[soln].toString());
  ...
  public static void checkTrace(TLCState[] trace, int traceLen) {
    ...
    for (int soln = 0; soln < solutions.length; soln++) {
      ...
      // RuntimeConfiguration.get().getErrStream().println(os.behavior.toString());
