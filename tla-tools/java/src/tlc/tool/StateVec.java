...
// Last modified on Fri Mar  2 15:37:34 PST 2001 by yuanyu
...
public final class StateVec {
  ...
  private static final TLCState[] emptyArr = new TLCState[0];
  ...
  public final void grow(int add) {
    int olen = this.v.length;
    if (olen >= TLCGlobals.setBound) {
      Assert.fail("Too many possible next states for the last state in the trace.");
    }
    int nlen = Math.min(Math.max(olen+add, 2*olen), TLCGlobals.setBound);
    TLCState old[] = this.v;
    this.v = new TLCState[nlen];
  ...
  /**
   * The next two methods should be used together when one wants to
   * clear a set of entries in a StateVec.  First, selected entries
   * are clearAt'd(), then a clearDone() is executed.
   */
  public final StateVec clearAt(int i) {
    // Assert.check(i < this.size);
    this.v[i] = null;
    return this;
  }

  public final StateVec clearDone() {
    int j = 0;
    for (int i = 0; i < this.size; i++) {
      if (this.v[i] != null)
	this.v[j++] = this.v[i];
