...
// Last modified on Wed Jan 10 00:09:44 PST 2001 by yuanyu
...
public class FPSetManager implements Serializable {
  ...
  public final boolean put(long fp) {
	...
	RuntimeConfiguration.get().getOutStream().println("Warning: Failed to connect from " + this.getHostName() +
			   " to the fp server at " + this.hosts[fpIdx] +
			   ".\n" + e.getMessage());
	if (this.reassign(fpIdx) == -1) {
	  RuntimeConfiguration.get().getOutStream().println("Warning: there is no fp server available.");	  
	  return false;
	}
      }
    }
  ...
  final class Checkpoint extends Thread {
	...
	RuntimeConfiguration.get().getErrStream().println("Error: Failed to checkpoint the fingerprint server at " +
