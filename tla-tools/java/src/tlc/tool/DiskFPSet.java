...
import tlc.util.ReadersWriterLock;
import tlc.util.Sort;
import util.Assert;
...
public class DiskFPSet extends FPSet {
	...
	tmpFile.delete();
        ...
        // merge
        this.mergeNewEntries(buff, raf, tmpRAF);
    ...
    public final void recover() throws IOException {
      // Close all "this.braf" and "this.brafPool" objects on currName:
      for (int i = 0; i < this.braf.length; i++) {
	this.braf[i].close();
      }
      for (int i = 0; i < this.brafPool.length; i++) {
      ...
      // We first recover the .fp file.
      this.recoverFromTrace();
      
      // We then initialize the in-memory index.
      if (this.fileCnt == 0) {
	this.index = null;
      }
      else {
	BufferedRandomAccessFile braf =
	    new BufferedRandomAccessFile(this.currName(), "r");
	int indexLen = (int)((this.fileCnt - 1) / NumEntriesPerPage) + 2;
	this.index = new long[indexLen];
	this.currIndex = 0;
	...
	long cnt = 0;
	while (true) {
	  this.index[this.currIndex++] = braf.readLong();
	  cnt += NumEntriesPerPage;
	  if (cnt >= this.fileCnt) {
	    braf.skipBytes((int)(this.fileCnt+NumEntriesPerPage-cnt-2)*8);
	    this.index[this.currIndex] = braf.readLong();
	    break;
	  }
	  braf.skipBytes(8192 - 8);
	}
	braf.close();
      }
    ...
    private final void recoverFromTrace() throws IOException {
      File fpFile = new File(this.currName());
      fpFile.delete();

      this.fileCnt = 0;
      long recoverPtr = TLCTrace.getRecoverPtr();
      int blen = (1 << 21);
      long[] buff = new long[blen];
      BufferedRandomAccessFile braf =
	new BufferedRandomAccessFile(TLCTrace.getFilename(), "r");

      while (braf.getFilePointer() < recoverPtr) {
	int idx = 0;
	while (idx < blen && braf.getFilePointer() < recoverPtr) {
	  braf.readLongNat();      /*drop*/
	  buff[idx++] = braf.readLong() & 0x7FFFFFFFFFFFFFFFL;
	  this.fileCnt++;
	}
	Sort.LongArray(buff, idx);
	this.mergeBuff(buff, idx, fpFile);
      }
      braf.close();
    }
