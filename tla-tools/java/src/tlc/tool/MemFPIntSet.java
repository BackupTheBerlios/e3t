...
public class MemFPIntSet extends FPIntSet {
  ...
  public synchronized final int setStatus(long fp, int status) {
	...
	if (fp == fp1) {
	  ...
	  // RuntimeConfiguration.get().getErrStream().println("setStatus: " + fp + "  " + status1 + " -> " + this.getStatus(fp));
	  return status1;
	}
      }
    }
    ...
    this.count++;
    // RuntimeConfiguration.get().getErrStream().println("setStatus: " + fp + "  NEW -> " + this.getStatus(fp));
    return NEW;
  }
