...
// Last modified on Thu Aug 23 17:46:39 PDT 2001 by yuanyu
...
import tlasany.parser.ASCII_CharStream;
import tlasany.parser.TLAplusParserConstants;
import tlasany.parser.TLAplusParserTokenManager;
import tlasany.parser.Token;
import tlasany.parser.TokenMgrError;
...
public class ModelConfig implements ValueConstants, Serializable {
  ...
  public final void parse() {
    ...
    try {
      ...
      ASCII_CharStream cs = new ASCII_CharStream(fis, 1, 1);
      TLAplusParserTokenManager tmgr = new TLAplusParserTokenManager(cs, 2);

      Token tt = getNextToken(tmgr);
      while (tt.kind != TLAplusParserConstants.EOF) {
	String tval = tt.image;
	int loc = cs.getBeginLine();
	if (tval.equals(Init)) {
	  ...
	  while ((tt = getNextToken(tmgr)).kind != TLAplusParserConstants.EOF) {
		...
		while (true) {
		  tt = getNextToken(tmgr);
		  Value arg = this.parseValue(tt, cs, tmgr);
		  line.addElement(arg);
		  tt = getNextToken(tmgr);
		...
		tt = getNextToken(tmgr);
		line.addElement(this.parseValue(tt, cs, tmgr));
  ...
  private final Value parseValue(Token tt, ASCII_CharStream cs,
  ...
  throws IOException {
    if (tt.kind == TLAplusParserConstants.NUMBER_LITERAL) {
      ...
      if (!tt.image.equals("}")) {
	while (true) {
	  Value elem = this.parseValue(tt, cs, tmgr);
  ...
  public static void main(String[] args) {
    try {
      ...
      while (t.kind != 0) {
	RuntimeConfiguration.get().getErrStream().println(t);
	t = getNextToken(tmgr);
      }
    }
    catch (Exception e) { RuntimeConfiguration.get().getErrStream().println(e.getMessage()); }
  }
