...
public class Combinatorics {
  ...
  public static void main(String argv[]) {
	...
	A[2*i+1] = new BigInteger(10, r);
      ...
      RuntimeConfiguration.get().getOutStream().println("A is "+print(A));
      b = toNum(B, A);
      C = toSeq(B, b);
      RuntimeConfiguration.get().getOutStream().println("C is "+print(C));
      for (i = 0; i < 16; i++)
	if (!C[i].equals(A[i]))
	    RuntimeConfiguration.get().getOutStream().println("***********Error***********");
      RuntimeConfiguration.get().getOutStream().println("b is " + b);
    }      
  }
