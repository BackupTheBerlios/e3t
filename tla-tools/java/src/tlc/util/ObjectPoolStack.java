...
// Last modified on Tue Feb 22 15:59:47 PST 2000 by yuanyu   
...
public class ObjectPoolStack {
  ...
  class Reader extends Thread {
	...
	RuntimeConfiguration.get().getErrStream().println("Error: when reading the disk (ObjectPoolStack.Reader.run):\n" +
			   e.getMessage());
	// System.exit(1);
      }
    }
  ...
  class Writer extends Thread {
	...
	RuntimeConfiguration.get().getErrStream().println("Error: when reading the disk (ObjectPoolStack.Writer.run):\n" +
			   e.getMessage());
	//System.exit(1);
      }
    }
